...
def get_js_common_functions():
    return r'''
      ...
      function formatMetricFormula(formula) {
        ...
        return 'Nsight Perf SDK metric(s): ' + formula;
      }
'''
    ...
    def jsfunc(gen):
      ...
      function tbody_AllCounters(tbody) {
        ...
        for (const counter of counterNames) {
          ...
          addCellSimple(trow, "ra", format_pct(pct), passThroughput = false, formatMetricFormula(pctStr));
          addCellSimple(trow, "la comp", toBarChart(pct, '█'), passThroughput = false, formatMetricFormula(pctStr));
          addCellSimple(trow, "ra", format_avg(avg), passThroughput = false, formatMetricFormula(avgStr));
          addCellSimple(trow, "ra", format_avg(apc, 2), passThroughput = false, formatMetricFormula(apcStr));
          addCellSimple(trow, "ra", format_avg(aps, 2), passThroughput = false, formatMetricFormula(apsStr));
          addCellSimple(trow, "ra", format_sum(sum), passThroughput = false, formatMetricFormula(sumStr));
          addCellSimple(trow, "ra", format_sum(sps), passThroughput = false, formatMetricFormula(spsStr));
    ...
    def jsfunc(gen):
      ...
      function tbody_AllRatios(tbody) {
        ...
        for (const ratio of ratioNames) {
          ...
          addCellSimple(trow, "ra", format_pct(pct, 2), passThroughput = false, formatMetricFormula(pctStr));
          addCellSimple(trow, "la comp", toBarChart(pct, '█'), passThroughput = false, formatMetricFormula(pctStr));
          addCellSimple(trow, "ra", format_avg(rio), passThroughput = false, formatMetricFormula(rioStr));
          addCellSimple(trow, "ra", format_avg(maxRate, 4), passThroughput = false, formatMetricFormula(maxRateStr));
          tbody.appendChild(trow);
        }
      }
'''
